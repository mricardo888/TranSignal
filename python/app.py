...
def fetch_supply_chain_news(company_key: str, max_articles: int = 10) -> list[dict]:
    ...
    supply chain exposure (supplier cities + supply chain terms).
    Results are cached for 5 minutes to avoid hammering the RSS endpoint.
    """
    ...
    for sup in erp["suppliers"].values():
        ...
        # Also extract the port city from transit_hub (e.g. "Port of Rotterdam" → "Rotterdam")
    ...
    try:
        ...
        pass   # network unavailable — silently return empty list
...
def run_full_pipeline(event: dict) -> None:
    ...
    add_log(f"[PERCEPTION]{src_tag} Geocoding disruption: {event['location']}…", "info")
    ...
    risk = assess_stockout_risk(transit["delay_added_days"], buf, inv["safety_stock_days"])
    ...
    add_log("[REASONING] Injecting memory context + ERP profile into Gemini prompt…", "info")
    result = run_agent(
    ...
    if result.get("_api_error"):
        add_log(f"[REASONING] ⚠ API error — showing mock fallback: {result['_api_error']}", "critical")
    else:
        add_log(f"[REASONING] Response received{src_note}.", "success")
    ...
    if hitl.get("requires_human_approval"):
        ...
        add_log("[ACTION] ✅ Autonomous action authorised — within threshold.", "success")
